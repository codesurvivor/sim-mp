 ...
 * setup project CMP
 *
 * Revision 1.1.1.1  2010/02/18 21:22:36  xue
 * CMP_NETWORK
 *
...
//   printf("pass 5_1_1_1\n");
...

  {
    ...
    printf("Thread 0 stack size (1)%d\n",  current->ld_stack_size);
	...
	if ((bfd_get_section_flags(abfd, sect) & SEC_ALLOC)
	    ...
	    && bfd_section_size(abfd, sect))
	  {
	    ...
	    mem_bcopy(mem_fn, Write, bfd_section_vma(abfd, sect), p, bfd_section_size(abfd, sect), current->id);
	...
	/* zero out the section if it is loadable but not allocated in exec */
		 ...
		 && bfd_section_size(abfd, sect))
	  {
	    ...
	    mem_bzero(mem_fn, bfd_section_vma(abfd, sect), bfd_section_size(abfd, sect), current->id);
	  }
	else
	  {
...
//   printf("pass 5_1_1_3_2\n");
	    mem_bcopy(mem_access, current->mem, Write, MD_SWAPQ(shdr.s_vaddr), p, MD_SWAPQ(shdr.s_size), current->id);
//   printf("pass 5_1_1_3_3\n");
...
#if 0
  printf("text = %llx (%d)", (unsigned long long)current->ld_text_base, (int)current->ld_text_size);
  printf("Data = %llx (%d)", (unsigned long long)current->ld_data_base, (int)current->ld_data_size);
  printf("Stack = %llx (%d)", (unsigned long long)current->ld_stack_base, (int)current->ld_stack_size);
#endif
