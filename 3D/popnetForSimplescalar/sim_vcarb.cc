				...
				if(cont_temp > 0) {
					...
					if(cont_temp > 1) {
						vc_win = vc_o_i_map[VC_type(i,j)][SRGen::wrg().
							flat_l(0, cont_temp)];
#ifdef	ROUTER_VC_PRIORITY
						long priority = input_module_.get_flit(vc_win.first, vc_win.second).flit_id();
						for(long k = 0; k < cont_temp; k++)
						{
							if(vc_win.first == vc_o_i_map[VC_type(i,j)][k].first && input_module_.get_flit(vc_o_i_map[VC_type(i,j)][k].first, vc_o_i_map[VC_type(i,j)][k].second).flit_id() < priority)
							{
								vc_win = vc_o_i_map[VC_type(i,j)][k];
								priority = input_module_.get_flit(vc_o_i_map[VC_type(i,j)][k].first, vc_o_i_map[VC_type(i,j)][k].second).flit_id();
							}
						}
#endif
					}
